...
function checkId() {
   ...
   var params = 'id=' + $('#id', frm).val();
    ...
    $.ajax({
      ...
      type: 'get',  //get, post
      cache: false, //부라우저의 캐시영역 사용 안함
      async:true, //true:비동기 통신
      dataType: 'json',  //응답 형식 : json, xml, html ...
      data: params, // 서버로 전달하는 데이터
      success: function(rdata) {   //통신 성공, 서버로부터 성공적으로 응답이 온 경우
        // alert(rdate);
      var msg = "";
      if(rdata.count > 0) {
        $('#modal_content').attr('class', 'alert-danger'); // CSS 변경
        msg = "이미 사용 중인 ID 입니다."
        $.cookie('checkId', 'FALSE'); // Cookie 기록
      } else {
        $('#modal_content').attr('class', 'alert-success'); // CSS 변경
        msg = "사용 가능한 ID 입니다."
